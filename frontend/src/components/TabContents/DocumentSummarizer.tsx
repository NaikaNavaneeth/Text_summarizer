...
import { uploadAndSummarizeDocument, downloadAsTxt, downloadAsPdf, downloadAsDocx } from '@/services/api';
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
...
const DocumentSummarizer = () => {
  ...
  const handleSummarize = async () => {
    if (!file) {
      setSummaryOutput('Please upload a document file first.');
      return;
    }
    
    setIsProcessing(true);
    try {
      const result = await uploadAndSummarizeDocument(
        file,
        summaryMethod,
        summaryLength,
        useOpenAI
      ...
      setSummaryOutput('An error occurred while summarizing the document. Please try again.');
    } finally {
      ...
      <Card className="border border-gray-200 shadow-md overflow-hidden bg-white rounded-xl">
        <CardContent className="p-6 md:p-8">
            ...
            <div className={`border-2 border-dashed ${fileName ? 'border-violet-300 bg-violet-50' : 'border-gray-300'} rounded-xl p-10 text-center hover:bg-gray-50 transition-colors cursor-pointer group`}>
              ...
              <label htmlFor="doc-upload" className="cursor-pointer block w-full h-full">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-xs text-gray-500 mt-2">.docx or .txt (up to 10MB)</p>
                  {isUploading && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 bg-gray-50 p-5 rounded-lg">
            <div>
              <Label htmlFor="summary-method-doc" className="block text-sm font-medium text-gray-700 mb-2">
              ...
              >
                <SelectTrigger id="summary-method-doc" className="border-gray-300 focus:ring-violet-300 focus:border-violet-400">
            ...
            <div>
              <Label htmlFor="summary-length-doc" className="block text-sm font-medium text-gray-700 mb-2">
                Summary Length
              </Label>
              <Select 
                value={summaryLength} 
              ...
              >
                <SelectTrigger id="summary-length-doc" className="border-gray-300 focus:ring-violet-300 focus:border-violet-400">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <Label htmlFor="doc-output" className="text-md font-medium text-gray-800">
                  Summary Output
                </Label>
              </div>
